                        ...
                        *self.inner.write() = MaybeGitlab::Loaded(gl.clone());
                        ...
                        for p in ps {
                            if let Some(d) = p.default_branch {
                                ...
                                let f = async move {
                                    gl.get_archive_id(p.id, &d).instrument(span).await
                                };
                                let _ = js.spawn(f);
                            }
                        }
...
impl GitLab {
    ...
    pub async fn new(cfg: GitlabConfig) -> Result<Self, gitlab::GitlabError> {
        ...
        let Some(url_str) = url.host_str() else {
            return Err(gitlab::GitlabError::UrlParse {
    ...
    pub fn new_background(cfg: GitlabConfig) -> GLInstance {
        let r = GLInstance {
            inner: std::sync::Arc::new(parking_lot::RwLock::new(MaybeGitlab::Loading)),
        };
        r.clone().load(cfg);
        r
    }
