...
impl GitLabOAuth {
    ...
    pub async fn callback(
        &self,
        request: AuthRequest,
    ) -> Result<AccessToken, impl std::error::Error> {
        use oauth2::AuthorizationCode;
        let code = AuthorizationCode::new(request.code);
        let req = unwrap!(oauth2::reqwest::ClientBuilder::new().build().ok());
        self.0
            .exchange_code(code)
            .request_async(&req)
    ...
    async fn get_projects_i(&self, token: String) -> Result<Vec<crate::Project>, super::Err> {
        ...
        let r = gitlab::api::projects::Projects::builder()
            .simple(true)
            .min_access_level(gitlab::api::common::AccessLevel::Developer)
            //.membership(false)
            .build()
    ...
    /// #### Errors
    pub async fn get_archive_id(
        &self,
        id: u64,
        token: String,
        branch: &str,
    ) -> Result<Option<ArchiveId>, super::Err> {
        self.get_archive_id_i(id, token, branch)
            .instrument(crate::REMOTE_SPAN.clone())
            .await
    }
    #[instrument(
...
impl GitLab {
    ...
    pub fn new_oauth(&self, redirect: &str) -> Result<GitLabOAuth, OAuthError> {
        ...
        let auth = oauth2::basic::BasicClient::new(ClientId::new(app_id.to_string()))
            .set_client_secret(ClientSecret::new(app_secret.to_string()))
            .set_auth_uri(auth_url)
            .set_token_uri(token_url)
            .set_redirect_uri(redirect_url);
        Ok(GitLabOAuth(auth, self.clone()))
    }
    ...
    ) -> Result<GitlabUser, reqwest::Error> {
        let client = reqwest::Client::new();
        let resp = client
            .get(format!("{}/api/v4/user", self.0.url))
