...
use flams_math_archives::{utils::path_ext::PathExt, LocalArchive, MathArchive};
...
pub async fn unzip_from_remote(
    ...
    if let Err(e) = unpack_with_callback(&mut tar, &dest, cont).await {
        tracing::error!("Error unpacking stream: {e}");
        let _ = tokio::fs::remove_dir_all(dest).await;
        return Err(());
    }
    let mh = crate::settings::Settings::get()
        .mathhubs()
        .first()
...
async fn unpack_with_callback<R: tokio::io::AsyncRead + Unpin, P: AsRef<std::path::Path>>(
    tar: &mut tokio_tar::Archive<R>,
    dst: P,
...
pub fn zip(
    ...
    let dir_path = archive.path().to_path_buf();
