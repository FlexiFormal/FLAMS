...
use flams_ontology::{
    content::modules::OpenModule, narration::documents::UncheckedDocument, DocumentRange, Unchecked,
};
...
impl OMDocResult {
    pub(crate) fn load_html_body(path: &Path, full: bool) -> Option<(Vec<CSS>, String)> {
        ...
        file.seek(SeekFrom::Start(u64::from(css_offset + 20)))
            .ok()?;
        file.read_exact(&mut css).ok()?;
        let css = bincode::serde::decode_from_slice(&css, bincode::config::standard())
            .ok()?
            .0;
        if full {
            ...
            file.seek(SeekFrom::Start(u64::from(inner_offset_real)))
                .ok()?;
    ...
    #[cfg(feature = "tokio")]
    pub(crate) async fn load_html_body_async(
        ...
        let css = bincode::serde::decode_from_slice(&css, bincode::config::standard())
            ...
            .0;
        if full {
            file.seek(SeekFrom::Start(u64::from(body_start)))
                .await
                .ok()?;
            let mut html = vec![0; body_len as usize];
    ...
    pub(crate) async fn load_html_full_async(path: std::path::PathBuf) -> Option<String> {
        ...
        file.seek(SeekFrom::Start(u64::from(html_start)))
            .await
            .ok()?;
    ...
    pub(crate) fn load_html_fragment(
        path: &Path,
        range: DocumentRange,
    ) -> Option<(Vec<CSS>, String)> {
        use std::io::{Seek, SeekFrom};
        ...
        let css = bincode::serde::decode_from_slice(&css, bincode::config::standard())
            .ok()?
            .0;
        file.seek(SeekFrom::Start(u64::from(html_start) + range.start as u64))
            .ok()?;
    ...
    pub(crate) fn load_reference<T: flams_ontology::Resourcable>(
        path: &Path,
        range: DocumentRange,
    ) -> eyre::Result<T> {
        use std::io::{Seek, SeekFrom};
    ...
    #[cfg(feature = "tokio")]
    pub(crate) async fn load_html_fragment_async(
        path: std::path::PathBuf,
        range: DocumentRange,
    ) -> Option<(Vec<CSS>, String)> {
        use std::io::SeekFrom;
    ...
    pub(crate) fn write(&self, path: &Path) {
        ...
        let css = err!(bincode::serde::encode_to_vec(
            css,
...
impl BuildArtifact for OMDocResult {
    ...
    fn load(_p: &std::path::Path) -> Result<Self, std::io::Error>
    where
        Self: Sized,
    {
        unreachable!()
