...
use super::{queue::{Queue, QueueState, RunningQueue, TaskMap}, BuildStep, BuildStepI, BuildTask, BuildTaskI, BuildTaskId, Dependency, TaskState};
...
impl Queue {
  ...
  pub(super) fn sort(map:&TaskMap,state:&mut RunningQueue) {
    ...
    while !tasks.is_empty() {
      ...
      for t in &tasks {
        ...
        for d in step.0.requires.read().iter() { match d {
            Dependency::Resolved { task, strict,step } if *strict || weak => {
                match *task.get_step(*step).unwrap_or_else(|| unreachable!()).0.state.read() {
      ...
      if changed {
          tasks.retain(|t| t.steps().iter().any(|s| *s.0.state.read() == TaskState::None));
      } else if weak {
          weak = false;
      } else {
  ...
  pub(super) fn get_next(&self) -> Option<(BuildTask,BuildTargetId)> {
    loop {
      ...
      let QueueState::Running(RunningQueue{queue,blocked,running,..}) = &mut *state  else {unreachable!()};
      if queue.is_empty() && blocked.is_empty() && running.is_empty() { return None }
      if let Some((i,target)) = queue.iter().enumerate().find_map(|(next,e)| {
        Self::can_be_next(e).map(|t| (next,t))
      }) {
        let Some(task) = queue.remove(i) else { unreachable!() };
        *task.get_step(target).unwrap_or_else(|| unreachable!()).0.state.write() = TaskState::Running;
  ...
  fn can_be_next(e:&BuildTask) -> Option<BuildTargetId> {
    let step = e.0.steps.iter().find(|step|
      *step.0.state.read() == TaskState::Queued
    )?;
    for d in &step.0.requires.read().0 {
      if let Dependency::Resolved { task, step, strict } = d {
        if *strict && *task.get_step(*step).unwrap_or_else(|| unreachable!()).0.state.read() == TaskState::Running {
          return None
        }
      }
    }
  ...
  pub(super) fn enqueue<'a,I:Iterator<Item=&'a SourceFile>>(
    ...
    let has_target = |f:&SourceFile,tgt:BuildTargetId| {
      f.target_state.get(&tgt).is_some_and(|t| !stale_only || matches!(t,FileState::Stale(_)|FileState::New))
    };
    let should_queue = |f:&SourceFile| {
      targets.iter().any(|t| has_target(f,*t))
    };
    let mut count = 0;
    ...
    for f in files.filter(|f| should_queue(f)) {
      ...
      let task = match map.map.entry(key) {
        Entry::Vacant(e) => {
          count += 1;
          let steps = targets.iter().filter(|t| has_target(f,**t)).map(
            |t| BuildStep(Arc::new(BuildStepI {
              target:*t,
          ...
          let task_i = Arc::new(BuildTaskI {
            ...
            source: match archive {
              Archive::Local(archive) => Either::Left(
                archive.source_dir().join(&*f.relative_path)
              )
            },
  ...
  fn process_dependencies(task:&BuildTask,map:&mut TaskMap) {
    for s in task.steps() {
      ...
      if let Some(v) = map.dependents.remove(&key) {
        for (d,i) in v {
          if let Some(t) = d.get_step(s.0.target) {
            ...
            for d in &mut deps.0 { 
              if let Dependency::Physical {task:ref t,strict} = d {
                if *t == key {
                  *d = Dependency::Resolved { task:task.clone(), step: i, strict:*strict };
                }
              }
            }
          }
      ...
      for dep in &mut s.0.requires.write().0 {
        if let Dependency::Physical {task:ref deptask,strict} = dep {
          if deptask.archive == *task.0.archive.archive_id() && deptask.rel_path == task.0.rel_path {
            continue
            // TODO check for more
          }
          let key = (deptask.archive.clone(),deptask.rel_path.clone());
          if let Some(deptasks) = map.map.get(&key) {
            if let Some(step) = deptasks.steps().iter().find(
              |bt| bt.0.target == s.0.target
            ) {
              step.0.dependents.write().push((task.0.id,s.0.target));
              *dep = Dependency::Resolved { task:deptasks.clone(), step: s.0.target, strict:*strict };
              continue
            }
            map.dependents.entry(deptask.clone()).or_default().push((task.clone(),s.0.target));
          }
        }
      }
    }
