...
impl Searcher {
  #[inline]#[must_use]
  pub fn get() -> &'static Self { &SEARCHER }
  ...
  pub fn reload(&self) {
    ...
    SPAN.in_scope(move || {
      let index = tantivy::index::Index::create_in_ram(SearchSchema::get().schema.clone());
      let writer = WriterWrapper(index.writer(MEMORY_SIZE).expect("Failed to instantiate search writer"));
      tracing::info_span!("Loading search indices").in_scope(move || {
          GlobalBackend::get().all_archives().par_iter()
            ...
            .for_each(|a| {
                ...
                if out.exists() && out.is_dir() {
                        ...
                        .filter(|entry| entry.file_name() == "tantivy")
                    {
                      ...
                      let Ok(v): Result<Vec<SearchIndex>,_> = bincode::serde::decode_from_reader(file, bincode::config::standard()) else {
      ...
      let Ok(r) = index.reader() else {
        tracing::error!("Failed to instantiate search reader");return
      };
      *reader = r;
      *old_index = index;
      drop(reader);
  ...
  pub fn with_writer<R>(&self,f:impl FnOnce(&mut tantivy::IndexWriter) -> Result<R,()>) -> Result<R,()> {
    SPAN.in_scope(move || {
  ...
  pub fn query(&self,s:&str,opts:QueryFilter,num_results:usize) -> Option<Vec<(f32,SearchResult)>> {
    SPAN.in_scope(move || {
