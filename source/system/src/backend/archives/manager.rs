...
use flams_ontology::{
    ...
    uris::{ArchiveId, ArchiveUriTrait, NameStep, PathURIRef, PathURITrait},
    Unchecked,
};
...
impl ArchiveManager {
    ...
    pub fn reinit<'a, R>(
        ...
        paths: impl IntoIterator<Item = &'a Path, IntoIter: DoubleEndedIterator>,
    ) -> R {
    ...
    pub(crate) fn load_module(
        ...
        let archive = path_uri.archive_id();
        let path = path_uri.path();
        self.with_archive(archive, |a| a.and_then(|a| a.load_module(path, name)))
    }
}
