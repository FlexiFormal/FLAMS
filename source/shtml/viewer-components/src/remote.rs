...
impl ServerFunArgs for URIArgs {
    ...
    type Extra = ();
    #[cfg(feature="csr")]
    fn as_params(():&Self::Extra) -> Cow<'static,str> {"".into()}
    #[cfg(any(feature="hydrate",feature="ssr"))]
    #[inline]
...
pub struct ServerConfig {
    ...
    get_solution:Cache<URIArgs,immt_ontology::narration::exercises::Solutions>,
}
...
impl ServerConfig {
    ...
    /// #### Panics
    #[inline]
    pub async fn solution(&self,uri:immt_ontology::uris::DocumentElementURI) -> Result<immt_ontology::narration::exercises::Solutions,String> {
...
struct BodyCache<
    Req:serde::Serialize+'static+std::hash::Hash+PartialEq+Eq,
    Resp:Clone+for<'a> serde::Deserialize<'a>+'static
> {
    #[cfg(any(feature="hydrate",feature="csr"))]
    cache:immt_utils::parking_lot::Mutex<immt_utils::prelude::HMap<Req,Resp>>,
    #[cfg(feature="csr")]
    url_frag:&'static str,
    #[cfg(any(feature="hydrate",feature="ssr"))]
    getter:std::sync::OnceLock<fn(Req) -> server_fun_ret!(Resp)>,
    #[cfg(feature="ssr")]
    phantom:std::marker::PhantomData<(Req,Resp)>
}
...
> BodyCache<Req,Resp> {
    ...
    /// #### Panics
    #[allow(unreachable_code)]
    pub async fn call(&self,key:Req) -> Result<Resp,String> {
        #[cfg(any(feature="hydrate",feature="csr"))]
        {{
            let cache = self.cache.lock();
            if let Some(v) = std::collections::HashMap::get(&*cache, &key) {
        ...
        #[cfg(feature="csr")]
        {
            let ret: Result<Resp,_> = ServerConfig::remote(self.url()).await;
            if let Ok(v) = &ret {
                let mut cache = self.cache.lock();
                cache.insert( key.clone(), v.clone());
            }
