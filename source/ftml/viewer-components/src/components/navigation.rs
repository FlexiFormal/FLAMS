    ...
    pub fn new() -> Self {
        ...
        let signal = RwSignal::new(
            window()
                ...
                .hash()
                .ok()
                .map(|s| s.strip_prefix('#').map(ToString::to_string).unwrap_or(s))
                .unwrap_or_default(),
        );
        ...
        {
            let on_hash_change =
                wasm_bindgen::prelude::Closure::wrap(Box::new(move |_e: leptos::web_sys::Event| {
                    let s = window()
                        .location()
                        .hash()
                        .ok()
            ...
            let on_anchor_click = wasm_bindgen::prelude::Closure::wrap(Box::new(
