...
mod term_replacing {
  ...
  use flams_ontology::{content::terms::ArgMode, narration::notations::{PresentationError, PresenterArgs}, uris::{DocumentElementURI, URI}};
...
#[cfg(feature="omdoc")]
pub(super) fn math_term(skip:usize,mut elements:ftml_extraction::prelude::FTMLElements,orig:OriginalNode,t:OpenTerm) -> impl IntoView {
    ...
    Either::Left({
      if use_context::<Option<SkipOne>>().flatten().is_some() {
        ...
        if subst {
          ...
          let notation_signal = 
            uri.as_ref().map(|(_,uri)| expect_context::<FTMLConfig>().get_forced_notation(&uri));
