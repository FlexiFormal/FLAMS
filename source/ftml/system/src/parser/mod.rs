...
use ftml_extraction::{errors::FTMLError, open::{terms::{OpenTerm, VarOrSym}, OpenFTMLElement}, prelude::{all_rules, Attributes, ExtractorState, RuleSet, FTMLElements, FTMLNode, FTMLTag, StatefulExtractor}};
...
struct Extractor<'a> {
  ...
  css:Vec<CSS>,
  refs:Vec<u8>,
  triples:HSet<flams_ontology::rdf::Triple>,
...
impl StatefulExtractor for Extractor<'_> {
  ...
  fn add_resource<T:flams_ontology::Resourcable>(&mut self,t:&T) -> LazyDocRef<T> {
...
impl TreeSink for HTMLParser<'_> {
            ...
            if let Some(child_elem) = child.as_element() {
              {
                ...
                if let Some(elements) = all_rules().applicable_rules(&mut *extractor, &mut *attributes) {
                  ...
                  child_elem.ftml.set(Some(elements));
                } else {
                  drop(attributes);
                  NodeRef::update_len(child_elem);
                }
  ...
  fn pop(&self, node: &Self::Handle) {
          ...
          if let Some(pe) = p.as_element() {
            if &pe.name.local == "head" {
              match &*elem.name.local {
                ...
                "style" => {
                  let str = node.children().filter_map(|c| c.as_text().map(|s| s.borrow().to_string())).collect::<String>();
      ...
      if let Some(mut elems) = elem.ftml.take() {
        let mut extractor = self.extractor.borrow_mut();
        elems.close(&mut *extractor,node);
        if !elems.is_empty() {
          elem.ftml.set(Some(elems));
        }
      }
  }
...
fn update_attributes(elements:&FTMLElements,child:&ElementData) {
  let mut attrs = child.attributes.borrow_mut();
  for e in &elements.elems { match e {
    OpenFTMLElement::ImportModule(uri) =>
      attrs.update(FTMLTag::ImportModule, uri),
    OpenFTMLElement::UseModule(uri) =>
      attrs.update(FTMLTag::UseModule, uri),
    ...
    OpenFTMLElement::Notation{symbol:VarOrSym::S(uri),..} => {
