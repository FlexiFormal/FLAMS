...
use ftml_extraction::{
    errors::FTMLError,
    open::{
        terms::{OpenTerm, VarOrSym},
        OpenFTMLElement,
    },
    prelude::{
        all_rules, Attributes, ExtractorState, FTMLElements, FTMLNode, FTMLTag, RuleSet,
        StatefulExtractor,
    },
};
use html5ever::{
...
impl StatefulExtractor for Extractor<'_> {
    ...
    fn add_resource<T: flams_ontology::Resourcable>(&mut self, t: &T) -> LazyDocRef<T> {
        ...
        let _ = bincode::serde::encode_into_writer(
            t,
            VecWriter(&mut self.refs),
        ...
        match child {
            NodeOrText::AppendNode(child) => {
                ...
                if parent.as_document().is_some() {
                    ...
                    let new_start =
                        parent_elem.end_offset.get() - nodes::tag_len(&parent_elem.name) - 1;
                    if let Some(child_elem) = child.as_element() {
                        {
                            let mut attributes = child_elem.attributes.borrow_mut();
                            let mut extractor = self.extractor.borrow_mut();
                            if let Some(elements) =
                                all_rules().applicable_rules(&mut *extractor, &mut *attributes)
                            {
                                drop(attributes);
