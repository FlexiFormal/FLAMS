use either::Either;
use flams_ontology::{
    content::terms::{ArgMode, Informal, Term},
    narration::notations::{NotationComponent, OpNotation},
};
use ftml_extraction::{
    open::OpenFTMLElement,
    prelude::{FTMLNode, NotationSpec},
};
use html5ever::{
    interface::ElemName,
        ...
        self.as_element().map_or_else(
            || {
                ...
                NotationSpec {
                    attribute_index: 5,
                    inner_index: 6,
                    components: vec![NotationComponent::S(ret.into())].into_boxed_slice(),
                    is_text: true,
                }
            },
            |n| {
                ...
                NotationSpec {
                    attribute_index,
                    inner_index,
                    components: ret.into_boxed_slice(),
                    is_text,
                }
            },
        )
    ...
    pub(super) fn do_term(&self) -> Term {
        if let Some(elem) = self.as_element() {
            if let Some(mut ftml) = elem.ftml.take() {
                if let Some(i) = ftml
                    .iter()
                    .position(|e| matches!(e, OpenFTMLElement::ClosedTerm(_)))
                {
                    let OpenFTMLElement::ClosedTerm(t) = ftml.elems.remove(i) else {
                        unreachable!()
                    };
                    return t;
                }
            ...
            let attrs = elem
                ...
                .map(|(k, v)| {
                    (
                        k.local.to_string().into_boxed_str(),
                ...
                match e {
                    OpenFTMLElement::Comp => {
                        if !ser.writer.is_empty() {
                            ret.push(NotationComponent::S(
                                String::from_utf8_lossy(&std::mem::take(ser.writer))
                                    .into_owned()
                                    .into_boxed_str(),
                            ));
                        }
                    ...
                    OpenFTMLElement::ArgSep => {
                        ...
                        ret.push(NotationComponent::ArgSep {
                            index: idx,
                            mode: new_mode,
                            sep: nret.into_boxed_slice(),
                        });
                        return (index, tp);
                    }
                    OpenFTMLElement::ArgMap => {
                        ...
                        ret.push(NotationComponent::ArgMap {
                            index: idx,
                            segments: nret.into_boxed_slice(),
                        });
                        return (index, tp);
                    }
                    // TODO ArgMapSep
...
fn get_is_text_and_offsets(e: &ElementData) -> (bool, u8, u16) {
    ...
    let i = e.attributes.borrow().len() as u16 + (u16::from(o) + 1);
    (t, o, i)
}
...
pub(super) fn filter_node_term(mut node: NodeRef) -> NodeRef {
    ...
    'outer: while let Some(e) = node.as_element() {
        ...
        if let Some(a) = e.ftml.take() {
            if a.iter()
                .any(|e| matches!(e, OpenFTMLElement::ClosedTerm(_)))
            {
                e.ftml.set(Some(a));
                return node;
            }
        ...
        let num_children = node
            .children()
            .filter(|n| {
                n.as_element().is_some()
                    || n.as_text().is_some_and(|t| !t.borrow().trim().is_empty())
            })
            .count();
