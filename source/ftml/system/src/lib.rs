...
use flams_system::{
    archive_kind,
    backend::{archives::LocalOut, AnyBackend, Backend},
...
archive_kind!(jupyterbk1 @ InventoriedArchive::create_new);
...
fn extract(backend: &AnyBackend, task: &BuildTask) -> BuildResult {
    let html: Result<HTMLString, _> =
        backend.with_local_archive(task.archive().archive_id(), |a| {
            let Some(a) = a else {
