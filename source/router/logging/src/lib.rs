...
impl LogSocket {
    ...
    fn convert(
        ...
        match e {
            ...
            LogTreeElem::Span(e @ LogSpan { closed: None, .. }) => LogEntrySignal::Span(
                next_id(),
                SpanSignal {
                    message: RwSignal::new(SpanMessage::Open {
                        name: e.name,
                    ...
                    level: e.level,
                    args: e.args,
                    children: RwSignal::new(
                        e.children
                            .into_iter()
                            .map(|e| Self::convert(e, warnings))
                            .collect(),
                    ),
                },
            ),
            LogTreeElem::Span(e) => {
                ...
                LogEntrySignal::Span(
                    next_id(),
                    SpanSignal {
                        message: RwSignal::new(SpanMessage::Closed(format!(
                            "{} (finished after {})",
                            e.name,
                            closed.since(e.timestamp)
                        ))),
                        target: e.target,
                        level: e.level,
                        args: e.args,
...
impl LogSignals {
    fn close(&self, id: NonZeroU64, timestamp: Timestamp) {
        if let Some(path) = self
            .open_span_paths
            .try_update_untracked(|v| v.remove(&id))
            .flatten()
        {
    ...
    fn close_i(sigs: RwSignal<Vec<LogEntrySignal>>, path: &[usize], timestamp: Timestamp) {
        ...
        sigs.try_with_untracked(|v| {
            if let Some(LogEntrySignal::Span(_, s)) = v.get(i) {
                if path.is_empty() {
                    s.message.try_update(|m| {
                        if let SpanMessage::Open {
                            name,
                            timestamp: old,
                        } = &m
                        {
                            let msg =
                                format!("{} (finished after {})", name, timestamp.since(*old));
            ...
            None => {
                if let Some(id) = is_span {
                    self.top.try_update(|ch| {
                        self.open_span_paths
                            .try_update_untracked(|v| v.insert(id, vec![ch.len()]));
                        ch.push(line);
                    });
                } else {
    ...
    fn merge_i(
        &self,
        sigs: RwSignal<Vec<LogEntrySignal>>,
        i: usize,
        mut path: Vec<usize>,
        is_span: Option<NonZeroU64>,
        line: LogEntrySignal,
    ) {
        if i == path.len() {
            return;
        ...
        if path.len() == i + 1 {
            sigs.try_update(|v| {
                if let Some(s) = is_span {
                    path.push(v.len());
                    self.open_span_paths
                        .try_update_untracked(|v| v.insert(s, path));
                }
                v.push(line);
            });
        } else {
            sigs.try_with_untracked(|v| {
                let LogEntrySignal::Span(_, s) = &v[e] else {
                    unreachable!()
                };
