...
async fn run<T: LspService<Response = serde_json::value::Value>>(
    socket: SocketWrapper,
    main: MainLoop<T>,
) where
    async_lsp::ResponseError: From<T::Error>,
{
...
impl SocketWrapper {
    fn poll_read_internal(
        ...
        r.map(|result| match result {
            ...
            Some(Ok(message)) => match message {
                Message::Text(text) => Ok(Self::handle_incoming_data(
                    buf,
                    text.as_bytes(),
                    &mut this.read_buf,
                )),
                Message::Binary(binary) => {
                    Ok(Self::handle_incoming_data(buf, &binary, &mut this.read_buf))
                }
    ...
    fn handle_incoming_data(buf: &mut [u8], data: &[u8], read_buf: &mut Vec<u8>) -> usize {
...
impl futures::AsyncBufRead for SocketWrapper {
    ...
    fn consume(self: Pin<&mut Self>, amt: usize) {
        let this = self.get_mut();
        this.read_buf
            .drain(..std::cmp::min(amt, this.read_buf.len()));
    }
}
...
impl futures::AsyncWrite for SocketWrapper {
    fn poll_write(
        ...
        let msg = match buf.to_vec().try_into() {
            Ok(m) => m,
            Err(e) => return Poll::Ready(Err(io::Error::new(ErrorKind::Other, e))),
        };
        ...
        match futures::Sink::poll_ready(inner, cx) {
            ...
            Poll::Ready(Ok(())) => Poll::Ready(
                futures::Sink::start_send(Pin::new(&mut *lock), message)
                    .map(|()| buf.len())
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        let mut lock = self.inner.lock();
        let inner = Pin::new(&mut *lock);
        futures::Sink::poll_flush(inner, cx).map_err(|e| io::Error::new(ErrorKind::Other, e))
    }
    ...
    fn poll_close(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        let mut lock = self.inner.lock();
        let inner = Pin::new(&mut *lock);
        futures::Sink::poll_close(inner, cx).map_err(|e| io::Error::new(ErrorKind::Other, e))
    }
}
