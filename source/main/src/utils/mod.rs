...
pub fn from_server_fnonce<E, Fut, F, T, V: IntoView + 'static>(
    needs_login: bool,
    f: F,
    ...
    if needs_login {
        Either::Left(move || {
            ...
            match LoginState::get() {
                ...
                | LoginState::User { is_admin: true, .. } => EitherOf3::B(go()),
...
pub fn from_server_clone<E, Fut, F, T, V: IntoView + 'static>(
    needs_login: bool,
    f: F,
    r: impl FnOnce(T) -> V + Clone + Send + 'static,
) -> impl IntoView
where
    Fut: Future<Output = Result<T, ServerFnError<E>>> + Send + 'static,
    F: Fn() -> Fut + Send + Sync + 'static,
...
pub fn from_server_copy<E, Fut, F, T, V: IntoView + 'static>(
    needs_login: bool,
    f: F,
    r: impl FnOnce(T) -> V + Copy + Send + 'static,
) -> impl IntoView
where
    Fut: Future<Output = Result<T, ServerFnError<E>>> + Send + 'static,
    F: Fn() -> Fut + Send + Sync + 'static,
...
pub fn needs_login<V: IntoView + 'static>(
    mut f: impl FnMut() -> V + Send + 'static,
) -> impl IntoView {
