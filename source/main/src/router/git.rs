...
#[cfg(feature = "ssr")]
pub(crate) fn get_oauth(
) -> Result<(flams_git::gl::auth::GitLabOAuth, String), ServerFnError<String>> {
    use flams_git::gl::auth::GitLabOAuth;
    let Some(session) = use_context::<axum_login::AuthSession<flams_router_login::db::DBBackend>>()
    else {
...
pub async fn get_archives(
    ...
    use flams_system::backend::{
        archives::Archive, AnyBackend, Backend, GlobalBackend, SandboxedRepository,
    };
    async fn get(
        oauth: GitLabOAuth,
        secret: String,
                        ...
                        for r in rs {
                            match r {
                                SandboxedRepository::Git {
                                    ...
                                    if let Some(e) = ret.iter_mut().find_map(|(p, id, e)| {
                                        if id == rid {
                                            Some(e)
                                        } else {
                                            None
...
pub async fn update_from_branch(
    ...
    use super::buildqueue::LoginQueue;
    use flams_system::backend::{
        archives::Archive, AnyBackend, Backend, GlobalBackend, SandboxedRepository,
    };
    use flams_system::building::{queue_manager::QueueManager, QueueName};
...
pub async fn clone_to_queue(
    ...
    use super::buildqueue::LoginQueue;
    use flams_system::backend::{archives::Archive, AnyBackend, Backend, SandboxedRepository};
    use flams_system::building::{queue_manager::QueueManager, QueueName};
    use flams_system::formats::FormatOrTargets;
    let (oauth, secret) = get_oauth()?;
    let login = LoginState::get_server();
...
pub async fn get_new_commits(
    ...
    tokio::task::spawn_blocking(move || {
        login
            .with_opt_queue(queue, |queue_id, queue| {
                let AnyBackend::Sandbox(backend) = queue.backend() else {
                    unreachable!()
                };
