...
use flams_web_utils::{
    components::{Header, LazySubtree, Tree, wait_and_then},
    inject_css,
};
use leptos::prelude::*;
...
fn group(a: ArchiveGroupData) -> impl IntoView {
    ...
    view! {
      <LazySubtree>
        ...
        {
          wait_and_then(move || super::server_fns::group_entries(Some(id.clone())),
          |(groups,archives)|
    ...
    view! {
      <LazySubtree>
        <Header slot>{header}</Header>
        {
          ...
          wait_and_then(
              move || super::server_fns::archive_entries(id.clone(), Some(rel_path.clone())),
              move |(dirs,files)|
