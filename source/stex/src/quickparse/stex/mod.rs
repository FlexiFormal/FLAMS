...
pub enum STeXAnnot {
  ...
  Defnotation{full_range: SourceRange<LSPLineCol>},
  Paragraph{
    kind:ParagraphKind,
    full_range:SourceRange<LSPLineCol>,
    name_range:SourceRange<LSPLineCol>,
...
impl STeXAnnot {
  fn from_tokens<I:IntoIterator<Item=STeXToken<LSPLineCol>>>(iter: I,mut modules:Option<&mut SmallVec<(ModuleURI,ModuleRules<LSPLineCol>),1>>) -> Vec<Self> {
    ...
    for t in iter {
      match t {
        STeXToken::Module { uri, name_range, sig, meta_theory, full_range, smodule_range, children,rules,opts } => {
          if let Some(ref mut m) = modules { m.push((uri.clone(),rules)) };
          v.push(Self::Module { uri, name_range, sig, meta_theory, full_range, smodule_range, 
            opts:cont!(opts),
            children:Self::from_tokens(children,None) 
          });
        }
        STeXToken::UseStructure { structure, structure_range, full_range, token_range } =>
          v.push(STeXAnnot::UseStructure { structure, structure_range, full_range, token_range }),
        STeXToken::ConservativeExt { uri, ext_range, full_range, children, extstructure_range } =>
          v.push(Self::ConservativeExt { uri, ext_range, full_range, children:Self::from_tokens(children,None),extstructure_range }),
        STeXToken::MathStructure { uri, extends, name_range, opts, full_range, children, mathstructure_range,.. } => {
          v.push(Self::MathStructure { uri, extends, name_range, opts:cont!(opts), full_range, children:Self::from_tokens(children,None),mathstructure_range });
        }
        STeXToken::MorphismEnv { uri, star,env_range, full_range, children, name_range, domain, domain_range, kind,.. } =>
          v.push(Self::MorphismEnv { uri, env_range, star,full_range, children:Self::from_tokens(children,None),name_range,domain,domain_range,kind }),
        STeXToken::InlineMorphism { full_range, token_range, name_range, uri, domain, domain_range, kind, assignments,.. } =>
          v.push(Self::InlineMorphism{ full_range, token_range,name_range,uri,domain,domain_range,kind,
        ...
        STeXToken::VariableMacro { name, sequence,argnum, orig, token_range, full_range } => 
          v.push(STeXAnnot::VariableMacro { name, argnum, sequence, orig, token_range, full_range }),
        STeXToken::Svar { name, full_range, token_range, name_range, arg_range } =>
          v.push(STeXAnnot::Svar { name, full_range, token_range, name_range, arg_range }),
        ...
        STeXToken::Symdecl { uri, main_name_range, token_range, full_range, parsed_args } =>
          v.push(STeXAnnot::Symdecl { uri, main_name_range, token_range, full_range, 
            parsed_args:cont!(parsed_args)
          }),
        STeXToken::TextSymdecl { uri, main_name_range, full_range, parsed_args, token_range } =>
          v.push(Self::TextSymdecl{uri,main_name_range, full_range,token_range,
            parsed_args:cont!(parsed_args)
          }),
        STeXToken::Definiens { uri, full_range, token_range, name_range } =>
          v.push(Self::Definiens{uri,full_range,token_range,name_range}),
        STeXToken::Defnotation{full_range} =>
          v.push(STeXAnnot::Defnotation{full_range}),
        STeXToken::Notation { uri, token_range, name_range, notation_args, full_range } =>
          v.push(STeXAnnot::Notation { uri,token_range,name_range,full_range,
            notation_args:cont!(notation_args),
          }),
        STeXToken::Symdef { uri, main_name_range, token_range, full_range, parsed_args } =>
          v.push(STeXAnnot::Symdef { uri, main_name_range, token_range, full_range, 
            parsed_args: cont!(parsed_args)
          }),
        STeXToken::Vardef { name, main_name_range, token_range, full_range, parsed_args } =>
          v.push(STeXAnnot::Vardef { name, main_name_range,  token_range, full_range, 
            parsed_args: cont!(parsed_args),
          }),
        STeXToken::Varseq { name, main_name_range, token_range, full_range, parsed_args } =>
          v.push(STeXAnnot::Varseq { name, main_name_range, token_range, full_range, 
            parsed_args: cont!(parsed_args),
          }),
        STeXToken::Symref { uri, full_range, token_range, name_range, text } =>
        ...
        STeXToken::InlineParagraph { kind, full_range, token_range, children_range, symbol, parsed_args, children } =>
          v.push(STeXAnnot::InlineParagraph { symbol,kind,full_range, token_range, children_range,
            parsed_args:cont!(parsed_args),
            children:Self::from_tokens(children,None)
          }),
        STeXToken::RenameDecl { uri, token_range, orig_range, name_range, macroname_range, full_range } =>
          v.push(STeXAnnot::RenameDecl {uri, token_range, orig_range, name_range, macroname_range, full_range}),
        STeXToken::Assign { uri, token_range, orig_range, full_range } =>
          v.push(STeXAnnot::Assign {uri, token_range, orig_range, full_range}),
...
pub enum AnnotIter<'a> {
  Module(std::iter::Chain<SModuleArgIter<'a,LSPLineCol,STeXAnnot>,std::slice::Iter<'a,STeXAnnot>>),
...
impl TreeLike for STeXAnnot {
  ...
  fn children(&self) -> Option<Self::RefIter<'_>> {
    ...
    match self {
      Self::Module { opts,children, .. } =>
        Some(AnnotIter::Module(SModuleArgIter::new(opts).chain(children.iter()))),
      ...
      Self::MathStructure { children, opts,.. } =>
        Some(AnnotIter::Structure(MathStructureArgIter::new(opts).chain(children.iter()))),
      Self::ConservativeExt { children,.. } |
      Self::MorphismEnv{children,..} =>
        Some(AnnotIter::Slice(children.iter())),
      Self::SemanticMacro{ .. } | Self::VariableMacro{..} | Self::ImportModule{ .. } |
      Self::UseModule{ .. } | Self::SetMetatheory{ .. } |
...
pub fn quickparse<'a,S:STeXModuleStore>(uri:&'a DocumentURI,source: &'a str,path:&'a Path,backend:&'a AnyBackend,store:S) -> STeXParseDataI {  
  ...
  STeXParseDataI { annotations, diagnostics, modules, dependencies: dependents }
}