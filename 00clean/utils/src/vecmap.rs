...
impl<K, V> VecMap<K, V> {
    ...
    pub fn get_or_insert_mut(&mut self, key: K, value: impl FnOnce() -> V) -> &mut V where K:PartialEq {
        let ret = if let Some((i,_)) = self.0.iter().enumerate().find(|(_, (k,_))| k == &key) {i} else {
...
impl<V,V2> PartialEq<VecSet<V2>> for VecSet<V> where V: PartialEq<V2> {
...
impl<V> VecSet<V> {
    ...
    pub fn insert(&mut self, value: V) where V:PartialEq {
