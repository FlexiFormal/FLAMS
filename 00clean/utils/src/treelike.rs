...
pub trait TreeChild<T:TreeLike> {
    fn children<'a>(&self) -> Option<T::RefIter<'a>> where Self:'a;
}
...
pub trait TreeLike:Sized{
    ...
    fn dfs_with_close<'a,R,SG,SL,Open: FnMut(&mut SG,Self::Child<'a>) -> Result<DFSContinuation<SL>,R>,Close:FnMut(&mut SG,SL) -> Result<(),R>>(
        &'a self,state:&mut SG,open:Open,close:Close
    ) -> Result<(),R> {
    ...
    fn display_nested<'a>(
        ...
        open:impl Fn(&Self::Child<'a>,&mut Indentor,&mut std::fmt::Formatter<'_>) -> Result<DFSContinuation<()>,std::fmt::Error>,
    ...
    #[inline]
    #[allow(clippy::missing_errors_doc)]
    fn display_children<'a,I:Into<Self::RefIter<'a>>>(i:I,f:&mut std::fmt::Formatter<'_>,
          open:impl Fn(&Self::Child<'a>,&mut Indentor,&mut std::fmt::Formatter<'_>) -> Result<DFSContinuation<()>,std::fmt::Error>,
          close:impl Fn(&Self::Child<'a>,&mut std::fmt::Formatter<'_>) -> std::fmt::Result,
          indent:Option<Indentor>
    ) -> std::fmt::Result  where Self:'a {
...
impl Display for Indentor<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.2.replace(false) || self.1 == 0 { return Ok(()) }
        f.write_char('\n')?;
        for _ in 0..self.1 { write!(f, "{}", self.0)?; }
        Ok(())
    }
}
...
pub trait TreeChildIter<'a,T:TreeLike<RefIter<'a>=Self>+'a>:Iterator where Self:Sized+'a {
    #[inline]
    ...
    fn dfs_with_close<R,SG,SL,Open: FnMut(&mut SG,T::Child<'a>) -> Result<DFSContinuation<SL>,R>,Close:FnMut(&mut SG,SL) -> Result<(),R>>(
        self,state:&mut SG,mut open:Open,mut close:Close
    ) -> Result<(),R> {
        fn i_next<'a,SL,T:TreeLike+'a>(current:&mut (Option<SL>,T::RefIter<'a>),stack:&mut Vec<(Option<SL>,T::RefIter<'a>)>) -> Option<(bool,T::Child<'a>)> {
        ...
        loop {
            let r = if let Some(r) = i_next::<SL,T>(&mut current,&mut stack) {
                ...
                loop { if let Some(next) = stack.pop() {
                    let (old, _) = std::mem::replace(&mut current, next);
                    if let Some(s) = old { close(state, s)? }
                    if let Some(c) = i_next::<SL, T>(&mut current, &mut stack) { break Some(c); }
                } else { break None } }
            };
            if let Some((has_children,r)) = r {
                match open(state,r)? {
                    DFSContinuation::Continue => (),
                    DFSContinuation::OnClose(s) if has_children => {current.0 = Some(s);},
                    DFSContinuation::OnClose(s) => close(state,s)?,
                    DFSContinuation::SkipChildren => {
                        if has_children { current = stack.pop().unwrap_or_else(|| unreachable!()) }
                    }
                    DFSContinuation::SkipNext => {current.1.next();}
    ...
    #[allow(clippy::missing_errors_doc)]
    fn display_nested(self,
        f:&mut std::fmt::Formatter<'_>,
        open:impl Fn(&T::Child<'a>,&mut Indentor,&mut std::fmt::Formatter<'_>) -> Result<DFSContinuation<()>,std::fmt::Error>,
        close:impl Fn(&T::Child<'a>,&mut std::fmt::Formatter<'_>) -> std::fmt::Result,
        ind: Option<Indentor>
    ) -> std::fmt::Result {
        let mut ind = ind.unwrap_or_default();
        self.dfs_with_close(&mut (f,&mut ind),
            |(f, ind), c| {
                ind.fmt(f)?;
...
mod tests {
    ...
    fn dfs() {
        let foo = foo();
        let dfs = foo.dfs().unwrap_or_else(|| unreachable!()).map(FooChild::name).collect::<Vec<_>>();
        assert_eq!(dfs,vec!["a","b","c","d","e","f","g","h","i"]);
    }
    #[test]
    fn bfs() {
        let foo = foo();
        let bfs = foo.bfs().unwrap_or_else(|| unreachable!()).map(FooChild::name).collect::<Vec<_>>();
  ...
  <Foo2>
    ...
    impl Display for TopFoo {
        fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
            write!(f,"<TopFoo>")?;
            self.display_nested(f,|c,_,f| match *c {
                FooChild::Leaf(s) => {
                    write!(f,"<Leaf {s}/>")?;
    ...
    impl<'a> TreeChild<TopFoo> for &'a FooChild {
        fn children<'b>(&self) -> Option<<TopFoo as TreeLike>::RefIter<'b>> where Self:'b {
            match self {
