...
mod private {
    ...
    impl<A:?Sized> Ptr for std::rc::Rc<A> {
        ...
        #[inline]
        fn inner(&self) -> &A { self }
        //#[inline]
        //fn new(a: A) -> Self { std::rc::Rc::new(a) }
        #[inline]
        fn value(&self) -> *const A { Self::as_ptr(self) }
        #[inline]
        fn weak(&self) -> Self::Weak { Self::downgrade(self) }
    }
    impl<A:?Sized> Weak for std::rc::Weak<A> {
        type A = A;
        type Strong = std::rc::Rc<A>;
        #[inline]
        fn upgrade_ptr(&self) -> Option<Self::Strong> { Self::upgrade(self) }
        #[inline]
        fn value(&self) -> *const A { self.as_ptr() }
        #[inline]
        fn is_alive(&self) -> bool { self.strong_count() > 0 }
    }
...
impl<P:Ptr> AsRef<P::A> for Interned<P> {
    #[inline]
    fn as_ref(&self) -> &P::A { self.0.inner() }
}
impl <P:Ptr> Hash for Interned<P> {
    #[inline]
    #[allow(clippy::ptr_as_ptr)]
    fn hash<H:Hasher>(&self, state: &mut H) { (self.0.value() as *const () as usize).hash(state) }
}
...
impl<P:Ptr> Ord for Interned<P> {
    #[inline]
    #[allow(clippy::ptr_as_ptr)]
    fn cmp(&self, other: &Self) -> std::cmp::Ordering { (self.0.value() as *const () as usize).cmp(&(other.0.value() as *const () as usize)) }
}
...
impl<P:Ptr> PartialEq for Interned<P> {
    ...
    #[allow(clippy::ptr_as_ptr)]
    #[allow(clippy::ptr_eq)]
    fn eq(&self, other: &Self) -> bool { (self.0.value() as *const () as usize) == (other.0.value() as *const () as usize) }
}
impl<P:Ptr> Eq for Interned<P> {}

impl<A:?Sized> AsRef<A> for Interned<triomphe::Arc<A>> {
    #[inline]
    fn as_ref(&self) -> &A { self.0.as_ref() }
}
impl <A:?Sized> Hash for Interned<triomphe::Arc<A>> {
    #[inline]
    #[allow(clippy::ptr_as_ptr)]
    #[allow(clippy::ref_as_ptr)]
    fn hash<H:Hasher>(&self, state: &mut H) { (self.as_ref() as *const _ as *const () as usize).hash(state)  }
}
...
impl<A:?Sized> PartialEq for Interned<triomphe::Arc<A>> {
    ...
    #[allow(clippy::ptr_as_ptr)]
    #[allow(clippy::ptr_eq)]
    fn eq(&self, other: &Self) -> bool { (self.as_ref() as *const _ as *const () as usize) == (other.as_ref() as *const _ as *const () as usize) }
}
...
impl<A:?Sized> Ord for Interned<triomphe::Arc<A>> {
    ...
    #[allow(clippy::ptr_as_ptr)]
    #[allow(clippy::ref_as_ptr)]
    fn cmp(&self, other: &Self) -> std::cmp::Ordering { (self.as_ref() as *const _ as *const () as usize).cmp(&(other.as_ref() as *const _ as *const () as usize)) }
}
...
impl<W:Weak,const N:usize,const GC:usize> GCInterner<W,N,GC> {
    ...
    pub fn get_or_intern<V>(&mut self,v:V) -> Interned<W::Strong> where for<'a> &'a W::A:PartialEq<V>, V:Hash + PartialEq + Eq + Into<W::Strong> {
        let hash = Self::hash(&v);
        let ret = match self.store.entry(hash) {
            std::collections::btree_map::Entry::Occupied(mut e) => {
                let vec = e.get_mut();
                let r = vec.iter().find_map(|weak| unsafe{
                    weak.value().as_ref().map_or_else(|| None,|p| {
                        if weak.is_alive() && p == v { Some(weak.upgrade_ptr()) } else { None }
                    }).flatten()
                });
                r.map_or_else(|| {
                    let p = v.into();
    ...
    pub fn get<V>(&self,v:&V) -> Option<Interned<W::Strong>> where for<'a> &'a W::A:PartialEq<V>, V:Hash + PartialEq + Eq {
        let hash = Self::hash(&v);
        self.store.get(&hash).and_then(|vec| {
            vec.iter().find_map(|weak| unsafe{
                weak.borrow().value().as_ref().map_or_else(|| None,|p| {
                    if weak.is_alive() && p == *v { Some(weak.upgrade_ptr()) } else { None }
                })
            })
        }).flatten().map(Interned)
    }
}
...
impl<T:?Sized+Hash,const N:usize,const GC:usize> GCInterner<triomphe::Arc<T>,N,GC> {
    ...
    pub fn get_or_intern<V>(&mut self,v:V) -> Interned<triomphe::Arc<T>> where for<'a> &'a T:PartialEq<V>, V:Hash + PartialEq + Eq + Into<triomphe::Arc<T>> {
        let hash = Self::hash(&v);
        let ret = match self.store.entry(hash) {
            std::collections::btree_map::Entry::Occupied(mut e) => {
                let vec = e.get_mut();
                let r = vec.iter_mut().find_map(|weak| {
                    if weak.as_ref() == v { Some(weak.clone()) } else { None }
                });
                r.map_or_else(|| {
                    let p = v.into();
                    vec.push(p.clone());
    ...
    pub fn get<V>(&self,v:&V) -> Option<Interned<triomphe::Arc<T>>> where for<'a> &'a T:PartialEq<V>, V:Hash + PartialEq + Eq {
        let hash = Self::hash(&v);
        self.store.get(&hash).and_then(|vec| {
            vec.iter().find_map(|weak| if weak.as_ref() == *v { Some(weak.clone()) } else { None })
        }).map(Interned)
    }
}

pub type RcInterner<T,const N:usize=4,const GC:usize=0> = GCInterner<std::rc::Weak<T>,N,GC>;
pub type ArcInterner<T,const N:usize=4,const GC:usize=0> = GCInterner<std::sync::Weak<T>,N,GC>;
pub type TArcInterner<T,const N:usize=4,const GC:usize=0> = GCInterner<triomphe::Arc<T>,N,GC>;
