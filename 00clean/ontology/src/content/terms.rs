...
impl Term {
     ...
     */
    fn display_top<const SHORT:bool>(&self,f:&mut Formatter<'_>,indent:Option<Indentor>) -> std::fmt::Result {
        if let Some(i) = &indent { i.skip_next()};
        Self::display_children(TermChildrenIter::One(self),f,
                               |t,i,f| t.display_start::<true>(i,f),
                               |t,f| t.display_short_end(f),
                               indent
        )
    }
    fn display_start<const SHORT:bool>(&self,ind:&mut Indentor,f:&mut Formatter<'_>) -> Result<DFSContinuation<()>,std::fmt::Error> {
        macro_rules! cont {
            ($t:expr) => {
...
impl TreeLike for Term {
    ...
    fn children(&self) -> Option<Self::RefIter<'_>> {
        use Term::*;
        match self {
            OMID(_)| OMV(_) | OML{tp:None,df:None,..} => None,
            Field { record, owner, .. } =>
                Some(owner.as_ref().map_or(
                    TermChildrenIter::One(record),
                    |owner| TermChildrenIter::Two(record,owner)
                )),
            OMA { head, args } => Some(TermChildrenIter::WithHead(head,args)),
            //Field { record, .. } => Some(TermChildrenIter::One(record)),
            OML { df:Some(df), tp:Some(tp), .. } => Some(TermChildrenIter::Two(tp,df)),
            OML { df:None, tp:Some(tp), .. } => Some(TermChildrenIter::One(tp)),
            OML { df:Some(df), tp:None, .. } => Some(TermChildrenIter::One(df)),
            Informal { terms, .. } => Some(TermChildrenIter::Slice(terms.iter()))
        }
    }
}
impl<'a> TreeChild<Term> for &'a Term {
    fn children<'b>(&self) -> Option<<Term as TreeLike>::RefIter<'b>> where Self: 'b {
...
#[cfg_attr(feature="serde", derive(serde::Serialize, serde::Deserialize))]
pub enum ArgMode {
    #[default] Normal,Sequence,Binding,BindingSequence
}
...
pub enum Informal {
    ...
    Node {
        tag:String,
        attributes:VecMap<Box<str>,Box<str>>,
        children:Vec<Informal>
    },
    Text(Box<str>)
}
...
mod tests {
    ...
    use crate::{content::terms::Term,oma,oms,oml,omv};
