...
pub use name::{NameStep, Name};
pub use base::BaseURI;
pub use archives::{ArchiveId,ArchiveURI,ArchiveURITrait,ArchiveURIRef};
pub use paths::{PathURI,PathURIRef,PathURITrait};
pub use errors::URIParseError;
pub use content::{ContentURI,ContentURIRef,ContentURITrait,modules::ModuleURI,symbols::SymbolURI};
pub use narrative::{NarrativeURI,NarrativeURIRef,NarrativeURITrait,documents::DocumentURI,document_elements::DocumentElementURI};
...
macro_rules! common {
    () => {
        ...
        fn to_iri(&self) -> crate::rdf::NamedNode {
            crate::rdf::NamedNode::new(
                immt_utils::escaping::IRI_ESCAPE.escape(self).to_string()
...
#[cfg(feature = "serde")]
pub trait URIOrRefTrait:Display+Debug+Clone+Hash+serde::Serialize+sealed::Sealed {
    common!{}
}
...
impl<'a,A:URITrait<Ref<'a>=&'a A> + URIWithLanguage> URIWithLanguage for &'a A {
    #[inline]
    fn language(&self) -> Language { A::language(self) }
}
...
impl From<BaseURI> for URI {
    #[inline]
    fn from(b:BaseURI) -> Self { Self::Base(b) }
}
impl From<ArchiveURI> for URI {
    #[inline]
    fn from(b:ArchiveURI) -> Self { Self::Archive(b) }
}
impl From<PathURI> for URI {
    #[inline]
    fn from(b:PathURI) -> Self { Self::Path(b) }
}
impl From<ContentURI> for URI {
    #[inline]
    fn from(b:ContentURI) -> Self { Self::Content(b) }
}
impl From<NarrativeURI> for URI {
    #[inline]
    fn from(b:NarrativeURI) -> Self { Self::Narrative(b) }
}
...
impl<'a> From<&'a BaseURI> for URIRef<'a> {
    #[inline]
    fn from(b:&'a BaseURI) -> Self { Self::Base(b) }
}
impl<'a> From<&'a ArchiveURI> for URIRef<'a> {
    #[inline]
    fn from(b:&'a ArchiveURI) -> Self { Self::Archive(b.archive_uri()) }
}
impl<'a> From<PathURIRef<'a>> for URIRef<'a> {
    #[inline]
    fn from(b:PathURIRef<'a>) -> Self { Self::Path(b) }
}
impl<'a> From<ArchiveURIRef<'a>> for URIRef<'a> {
    #[inline]
    fn from(b:ArchiveURIRef<'a>) -> Self { Self::Archive(b) }
}
impl<'a> From<ContentURIRef<'a>> for URIRef<'a> {
    #[inline]
    fn from(b:ContentURIRef<'a>) -> Self { Self::Content(b) }
}
impl<'a> From<NarrativeURIRef<'a>> for URIRef<'a> {
    #[inline]
    fn from(b:NarrativeURIRef<'a>) -> Self { Self::Narrative(b) }
}
...
impl URI {
    fn parse_content(s:&str,module:&str,(language,next):(Language,Option<&str>),path:impl FnOnce() -> PathURI,mut split:Split<char>) -> Result<ContentURI,URIParseError> {
        ...
        next.strip_prefix(concatcp!(SymbolURI::SEPARATOR,"=")).map_or_else(
    ...
    fn parse_narrative(s:&str,document:&str,(language,next):(Language,Option<&str>),path:impl FnOnce() -> PathURI,mut split:Split<char>) -> Result<NarrativeURI,URIParseError> {
        ...
        next.strip_prefix(concatcp!(DocumentElementURI::SEPARATOR,"=")).map_or_else(
            || Err(URIParseError::UnrecognizedPart {
                original:s.to_string()
            }),
            |element| {
...
impl FromStr for URI {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let Some(next) = split.next() else {unreachable!()};
        next.strip_prefix(concatcp!(ArchiveURI::SEPARATOR,"=")).map_or_else(
            || Err(URIParseError::UnrecognizedPart {
                original:s.to_string()
            }),
            |archive| {
                ...
                let (path,next) = if let Some(path) = next.strip_prefix(concatcp!(PathURI::SEPARATOR,"=")) {
                    (Either::Left(|| PathURI { archive: archive(), path: Some(path.into()) }),split.next())
                } else {
                    (Either::Right(|| PathURI { archive: archive(), path: None }),Some(next))
                };
                ...
                let mut language = || {
                    split.next().map_or_else(|| Ok((Language::default(),None)),|n| {
                        n.strip_prefix(concatcp!(Language::SEPARATOR,"=")).map_or_else(
                            || Ok((Language::default(),Some(n))),
                            |l| (l.parse().map_err(|()| URIParseError::InvalidLanguage {
                                uri_kind:"uri",
                                original:s.to_string(),
                            }).map(|l| (l,split.next())))
                        )
                ...
                if let Some(module) = next.strip_prefix(concatcp!(ModuleURI::SEPARATOR,"=")) {
                    Ok(Self::Content(Self::parse_content(s,module,language()?,path,split)?))
                } else if let Some(document) = next.strip_prefix(concatcp!(DocumentURI::SEPARATOR,"=")) {
                    Ok(Self::Narrative(Self::parse_narrative(s,document,language()?,path,split)?))
                } else {
                    Err(URIParseError::UnrecognizedPart {
                        original:s.to_string()
                    })
...
impl<'a> URIRefTrait<'a> for URIRef<'a> {
    type Owned = URI;
    fn to_owned(self) -> URI {
        match self {
            Self::Base(b) => URI::Base(b.clone()),
            Self::Archive(a) => URI::Archive(a.to_owned()),
            Self::Path(p) => URI::Path(p.to_owned()),
            Self::Content(c) => URI::Content(c.to_owned()),
            Self::Narrative(n) => URI::Narrative(n.to_owned()),
        }
    }
}
...
#[cfg(feature = "serde")]
mod serde_impl {
    use super::{serialize, URI, URIRef};
    serialize!(DE URI);
    serialize!(URIRef<'_>);
}