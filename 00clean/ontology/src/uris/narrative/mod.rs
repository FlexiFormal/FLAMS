...
use crate::uris::{ArchiveURIRef, ArchiveURITrait, BaseURI, ContentURI, ContentURIRef, ContentURITrait, debugdisplay, ModuleURI, PathURITrait, SymbolURI, URIOrRefTrait, URIParseError, URIRef, URIRefTrait, URITrait, URIWithLanguage};
...
/*impl<'a,A:NarrativeURITrait+URITrait> URIWithLanguage for &'a A {
    #[inline]
    fn language(&self) -> Language {
        A::language(*self)
    }
}*/
impl<'a,A:NarrativeURITrait+URITrait<Ref<'a>=&'a A>> NarrativeURITrait for &'a A {
    #[inline]
    fn as_narrative(&self) -> NarrativeURIRef {
        A::as_narrative(*self)
    }
...
impl NarrativeURITrait for NarrativeURIRef<'_> {
    #[inline]
    fn as_narrative(&self) -> NarrativeURIRef { *self }
    #[inline]
    fn document(&self) -> &DocumentURI {
        match self {
            Self::Document(m) => m,
...
impl FromStr for NarrativeURI {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        DocumentURI::pre_parse(s, "narrative uri", |document, mut split| {
            let Some(c) = split.next() else { return Ok(Self::Document(document)) };
            c.strip_prefix(concatcp!(DocumentElementURI::SEPARATOR,"=")).map_or_else(
                || Err(URIParseError::TooManyPartsFor {
                    uri_kind: "narrative uri",
                    original: s.to_string()
                }),
                |name| Ok(Self::Element(DocumentElementURI { document, name: name.into() })
                )
            )
        })
    }
...
#[cfg(feature = "serde")]
mod serde_impl {
    use crate::uris::{NarrativeURI, NarrativeURIRef, serialize};
